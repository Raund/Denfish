...
class BlockViewed extends Module
{
	...
	{
		return (parent::install() && $this->registerHook('header') && $this->registerHook('home') && Configuration::updateValue('PRODUCTS_VIEWED_NBR', 4));
		...
		if (count($productsViewed))
		{
			...
			foreach ($productsViewed as $productViewed)
			{
				...
				else
				{
                    ...
                    $obj->price = Product::getPriceStatic($obj->id, true, null, 6);
                    $obj->price_tax_exc = Tools::ps_round($obj->price, 2);
                    $obj->quantity = Product::getQuantity($obj->id);
