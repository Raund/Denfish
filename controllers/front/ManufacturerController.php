...
class ManufacturerControllerCore extends FrontController
{
	...
	protected function assignOne()
	{
        ...
        $products = $this->manufacturer->getProducts($this->manufacturer->id, $this->context->language->id, (int)$this->p, 500, $this->orderBy, $this->orderWay);
        ...
        foreach($products as $product){
            ...
            if($category_id != 175){ // Без новинок
                if(!is_array($categories[$product['id_category_default']])){
                    $c_ids[] = $category_id;
                    $category_info = Category::getCategoryInformations($c_ids);
                    $categories[$category_id] = array(
                        'name' => $category_info[$category_id]['name'],
                        'count' => 0,
                        'products' => array()
                    );
                }
                $categories[$product['id_category_default']]['products'][] = $product;
