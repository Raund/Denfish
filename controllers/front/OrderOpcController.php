					...
					switch (Tools::getValue('method'))
					{
                        ...
                        case 'createOrder':
                            if ($this->context->customer->isLogged(true))
                            {
                                ...
                                //echo '=>' . $summary['total_price']; exit;

                                $order = new Order();
                                $order->id_address_delivery = (int)(Tools::getValue('id_address_delivery'));
                                $order->id_address_invoice = (int)(Tools::getValue('id_address_invoice'));
                                $order->id_cart = $this->context->cart->id;
                                $order->id_currency = $this->context->cart->id_currency;
                                $order->id_shop_group = $this->context->cart->id_shop_group;
                                $order->id_shop = $this->context->cart->id_shop;
                                $order->id_lang = $this->context->cart->id_lang;
                                $order->id_customer = $this->context->cart->id_customer;
                                $order->id_carrier = (int)(Tools::getValue('id_carrier'));
                                $order->total_discounts = $summary['total_discounts'];
                                $order->total_discounts_tax_exc = $summary['total_discounts_tax_exc'];
                                $order->total_price = $summary['total_price'];
                                $order->total_price_without_tax = $summary['total_price_without_tax'];
                                $order->total_products = $summary['total_products'];
                                $order->total_products_wt = $summary['total_products_wt'];
                                $order->total_shipping = $summary['total_shipping'];
                                $order->total_shipping_tax_exc = $summary['total_shipping_tax_exc'];
                                $order->total_tax = $summary['total_tax'];
                                $order->total_wrapping = $summary['total_wrapping'];
                                $order->total_wrapping_tax_exc = $summary['total_wrapping_tax_exc'];
                                $order->payment = Tools::getValue('payment');
                                $order->module = Tools::getValue('module');
                                $order->total_paid = $summary['total_price'];
                                $order->total_paid_tax_incl = $summary['total_price'];
                                ...
                                //$order->p
                                try{
                                    //echo $order->save(); echo $order->id; exit;
                                    if($order->save()){
                                        foreach($summary['products'] as $product){

                                        }
                                        $od = new OrderDetail();
                                        $od->createList($order, $this->context->cart, 3, $summary['products']);

                                        $idOrder = $order->id;
                                        //$msgText = Tools::getValue('msgText');
                                        $msgText = 'Оформлен новый заказа';
