...
<div class="box my_box">
	<h1 class="page-subheading">{l s='Адрес доставки'}</h1>
	<p class="info-title">
		{if isset($id_address) && (isset($smarty.post.alias) || isset($address->alias))}
			...
			{/if}
		{else}
			{l s='Что бы указать адрес доставки, пожалуйста заполните поля ниже.'}
		{/if}
	</p>
	{include file="$tpl_dir./errors.tpl"}
	<p class="required"><sup>*</sup>{l s='обязательно к заполнению'}</p>
	<form action="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" method="post" class="std" id="add_address">
		...
		{foreach from=$ordered_adr_fields item=field_name}
			...
			{if $field_name eq 'firstname'}
				<div class="required form-group">
					...
					<label for="firstname">{l s='Имя'} <sup>*</sup></label>
				</div>
			{/if}
			{if $field_name eq 'lastname'}
				<div class="required form-group">
					...
					<label for="lastname">{l s='Фамилия'} <sup>*</sup></label>
				</div>
			{/if}
			{if $field_name eq 'address1'}
				<div class="required form-group">
					...
					<label for="address1">{l s='Служба доставки'} <sup>*</sup></label>
				</div>
			{/if}
			{if $field_name eq 'address2'}
				<div class="required form-group">
					...
					<label for="address2">{l s='Отделение №'}<sup>*</sup></label>
				</div>
			{/if}
			...
			{if $field_name eq 'city'}
				<div class="required form-group">
					...
					<label for="city">{l s='Город'} <sup>*</sup></label>
				</div>
			...
			{if $field_name eq 'Country:name' || $field_name eq 'country'}
				<div class="required form-group" style="display:none;">
					...
					<label for="id_country">{l s='Страна'} <sup>*</sup></label>
				</div>
			{/if}
			{if $field_name eq 'State:name'}
				{assign var="stateExist" value=true}
				<div class="required id_state form-group">
					<label for="id_state">{l s='Область'} <sup>*</sup></label>
			...
			{if $field_name eq 'phone'}
				...
				<div class="form-group phone-number">
					...
					<label for="phone">{l s='Номер телефона'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>**</sup>{/if}</label>
				</div>
				{if isset($one_phone_at_least) && $one_phone_at_least}
					{assign var="atLeastOneExists" value=true}
					<p class="inline-infos required">** {l s='Вы должны указать хотя бы один номер'}</p>
				{/if}
			...
			{if $field_name eq 'phone_mobile'}
				...
				<div class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group">
					...
					<label for="phone_mobile">{l s='Дополнительный номер телефона'}{if isset($one_phone_at_least) && $one_phone_at_least} <sup>**</sup>{/if}</label>
				</div>
			{/if}
		{/foreach}
		<!--{if !$postCodeExist}
		...
		<div class="form-group">
			...
			<textarea class="validate form-control" data-validate="{$address_validation.other.validate}" id="other" name="other"  >{if isset($smarty.post.other)}{$smarty.post.other}{else}{if isset($address->other)}{$address->other|escape:'html':'UTF-8'}{/if}{/if}</textarea>
			<label for="other">{l s='Дополнительная информация'}</label>
		</div>
		{if !$homePhoneExist}
			<div class="form-group phone-number">
		...
		<div class="required form-group" id="adress_alias">
			...
			<label for="alias">{l s='Укажите имя шаблона адреса для подальшего использования'} <sup>*</sup></label>
		</div>
		<p class="submit2">
...
<ul class="footer_links clearfix">
	<li>
		<a class="btn btn-defaul button button-small" href="{$link->getPageLink('addresses', true)|escape:'html':'UTF-8'}">
			<span><i class="icon-chevron-left"></i> {l s='Назад к адресам'}</span>
		</a>
	</li>
</ul>
{strip}
