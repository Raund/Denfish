...
<div id="content_cart">
{capture name=path}{l s='Корзина'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}
<div id="center_column">

<h1 id="cart_title" class="page-heading"><!--{l s='Shopping-cart summary'}-->
	{if !isset($empty) && !$PS_CATALOG_MODE}
		<span class="heading-counter">{l s='Всего в корзине:'}
			<span id="summary_products_quantity">{$productNumber} 
			</span>
            <span>{l s='Отложено покупок:'}<!--Вписати змінну для підрахунку отлож покупок-->
                <span id="summary_products_quantity_wishlist">{$product.quantity|intval}
			</span>
			<!--{if $productNumber == 1}{l s='товар'}{/if}
			{if ($productNumber > 1 and $productNumber < 5)}{l s='товара'}{/if}
			{if $productNumber > 4}{l s='товаров'}{/if}-->
		</span>
	{/if}
</h1>
...
{if isset($empty)}
	<p class="alert alert-warning">{l s='Корзина пуста'}</p>
{elseif $PS_CATALOG_MODE}
	<p class="alert alert-warning">{l s='This store has not accepted your new order.'}</p>
{else}
	<p style="display:none" id="emptyCartWarning" class="alert alert-warning">{l s='Корзина пуста'}</p>
	...
	<p class="cart_navigation clearfix">
		...
		<a
			...
			>
			<i class="fa fa-angle-left fa-lg"></i>{l s='Продолжить покупки'}
		</a>
