...
<div class="box box-small clearfix">
	<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
			...
			<input type="hidden" value="" name="submitReorder"/>
				...
				<a href="#" onclick="$(this).closest('form').submit(); return false;" class="button btn btn-default button-medium pull-right"><span>{l s='Изменить заказ'}<i class="icon-chevron-right right"></i></span></a>
			<p class="dark">
				<strong>{l s='Ваш код заказа %s - сделан' sprintf=$order->getUniqReference()} {dateFormat date=$order->date_add full=0}</strong>
			</p>
...
<div class="info-order box">
	{if $carrier->id}<p><strong class="dark">{l s='Служба доставки:'}</strong> {if $carrier->name == "0"}{$shop_name|escape:'html':'UTF-8'}{else}{$carrier->name|escape:'html':'UTF-8'}{/if}</p>{/if}
	<p><strong class="dark">{l s='Способ оплаты:'}</strong> <span class="color-myaccount">{$order->payment|escape:'html':'UTF-8'}</span></p>
	...
	<div class="row">
		<div class="col-xs-12 col-sm-6"{if $order->isVirtual()} style="display:none;"{/if}>
			<ul class="address alternate_item box">
				<li><h3 class="page-subheading">{l s='Адрес доставки'} ({$address_delivery->alias})</h3></li>
...
<div id="order-detail-content" class="table_block table-responsive">
		<h3 class="page-heading bottom-indent">{l s='Детали заказа'}</h3>

	<table class="table table-bordered">
		<thead>
			<tr>
				{if $return_allowed}<th class="first_item"><input type="checkbox" /></th>{/if}
				<th class="{if $return_allowed}item{else}first_item{/if}">{l s='Код товара'}</th>
				<th class="item">{l s='Товар'}</th>
				<th class="item">{l s='Количество'}</th>
				{if $order->hasProductReturned()}
					<th class="item">{l s='Возвращено'}</th>
				{/if}
				<th class="item">{l s='Цена за ед.'}</th>
				<th class="last_item">{l s='Общая цена'}</th>
			</tr>
		</thead>
		<tfoot>
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td class="return_quantity">
							<input class="order_qte_input form-control grey" name="order_qte_input[{$product.id_order_detail|intval}]" type="text" size="2" value="{$productQuantity|intval}" style="display:none" />
							<div class="clearfix return_quantity_buttons">
								<a href="#" class="return_quantity_down btn btn-default button-minus"><span><i class="icon-minus"></i></span></a>
								<a href="#" class="return_quantity_up btn btn-default button-plus"><span><i class="icon-plus"></i></span></a>
							</div>
