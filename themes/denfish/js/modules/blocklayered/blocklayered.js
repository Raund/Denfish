...
function createButtonMoreProducts(){
    /*// удаляем предидущую кнопку
    var prevButtons = document.querySelectorAll('.block_product_next_box');
    for (var i = 0; i < prevButtons.length; i++) {
        prevButtons[i].remove();
    }*/

    var li = document.createElement('li');
    li.className = 'block_product_next_box';

    var a = document.createElement('a');
    a.href = '#';

    var span = document.createElement('span');
    span.innerHTML = '<img class="img_more" src="http://denfish.com/themes/denfish/img/more.png" alt="">Показать еще товары';

    li.appendChild(a);
    a.appendChild(span);

    var lastCurrentPage = parseInt($('div.pagination li.current').children().children().html());

    a.onclick = function(e){
        e.preventDefault();

        $('.block_product_next_box a img').css({
            //for firefox
        ...
        currentPages.push(lastCurrentPage);

        //currentPage++;
        p = '&p='+ (lastCurrentPage+1);

        reloadContent(p, true);
    };

    document.querySelector('ul.product_list').appendChild(li);
}

function paginationButton(nbProductsIn, nbProductOut) { // .block_product_next_box a
	...
	$('div.pagination li').not('.current, .disabled').each(function () {
		var nbPage = 0, notClick = false;
		if ($(this).hasClass('pagination_next')){
			nbPage = parseInt($('div.pagination li.current').children().children().html())+ 1;
            createButtonMoreProducts();
        }else if ($(this).hasClass('pagination_previous')){
            if(!currentPages.length){
                nbPage = parseInt($('div.pagination li.current').children().children().html())- 1;
            }else{
                nbPage = currentPages[0] - 1;
                if(!nbPage){
            ...
            var thisPage = parseInt($(this).children().children().html());
            var length = currentPages.length;
            if(length){
                for(var i=0; i<length; i++){
                    if(currentPages[i] == thisPage){
                        $(this).addClass('active-too');
                        notClick = true;
                    }
                }
            }
...
function reloadContent(params_plus, append)
{
	stopAjaxQuery();
	...
	ajaxQuery = $.ajax(
	{
		...
		success: function(result)
		{
			...
			if (result.productList){
                if(append){
                    $('.product_list .x-layered_ajax_loader').remove();
                    var productList = document.createElement('div');
                    productList.innerHTML = utf8_decode(result.productList);

                    var ul = productList.querySelector('ul.product_list');

                    console.log(ul);

                    document.querySelector('.block_product_next_box').remove();
                    document.querySelector('.product_list').innerHTML += ul.innerHTML;
