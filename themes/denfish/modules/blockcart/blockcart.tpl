...
{/if}
	<div class="shopping_cart">
		<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
			...
			<!--<span class=" price ajax_cart_quantity{if $cart_qties == 0} unvisible{/if}"style="display:none">{$cart_qties}</span>
			<span class="ajax_cart_product_txt{if $cart_qties != 1} unvisible{/if}" style="display:none">{l s='Product' mod='blockcart'}</span>
			<span class="ajax_cart_product_txt_s{if $cart_qties < 2} unvisible{/if}" style="display:none">{l s='Products' mod='blockcart'}</span>
			<span class="ajax_cart_total{if $cart_qties == 0} unvisible{/if}" style="display:none">
				{if $cart_qties > 0}
					{if $priceDisplay == 1}
			...
			<span class="ajax_cart_no_product{if $cart_qties > 0} unvisible{/if}" style="display:none"></span>
		...
		{if !$PS_CATALOG_MODE}
			<div class="cart_block block exclusive">
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}"style="display:none">
						{if $products}
							<dl class="products">
						...
						<div class="cart-prices">
							...
							<div class="cart-prices-line last-line"style="display:none">
								<span class="price cart_block_total ajax_block_cart_total" style="display:none">{$total}</span>
	...
	<div id="layer_cart">
		<div class="clearfix">
			<div class="layer_cart_product col-xs-12 col-md-6">
				<span class="cross" title="{l s='Close window' mod='blockcart'}"><i class="fa fa-times-circle fa-2x"></i></span>
				<h2>
					<i class="icon-ok"></i>{l s='Товар успешно добавлен в корзину' mod='blockcart'}
				</h2>
			...
			<div class="layer_cart_cart col-xs-12 col-md-6">
				...
				<div class="button-container">	
					<span class="continue btn btn-default button exclusive-medium" title="{l s='Continue shopping' mod='blockcart'}">
						<span>
							<i class="icon-chevron-left left"></i>{l s='Продолжить покупки' mod='blockcart'}
						</span>
					</span>
					<a class="btn btn-default button button-medium"	href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart'}" rel="nofollow">
						<span>
							{l s='Оформить заказ' mod='blockcart'}<i class="icon-chevron-right right"></i>
						</span>
					</a>	
				</div>
			</div>
